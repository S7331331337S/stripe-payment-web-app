...
export default async function OrderCompletePage({
  ...
  return (
    <main className="flex min-h-screen items-center justify-center bg-background px-5 py-16 text-foreground sm:px-8">
      <div className="w-full max-w-xl rounded-[2rem] border border-white/80 bg-white/75 p-8 text-center shadow-[0_24px_70px_-30px_rgba(42,54,92,0.35)] backdrop-blur-xl sm:p-12">
        ...
        {outcome.kind === 'paid' ? (
          <>
            ...
            <p className="mt-4 text-sm leading-6 text-slate-600">
              ...
              <span className="font-semibold text-slate-900">${outcome.amount}</span>.
              {outcome.email ? (
                <>
                  {' '}
                  A Stripe receipt is on its way to{' '}
                  <span className="font-medium text-slate-900">{outcome.email}</span>.
                </>
              ) : null}
            </p>
