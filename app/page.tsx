        ...
        <div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-3 sm:px-8">
          ...
          <div className="flex items-center gap-2">
            ...
            <nav aria-label="Primary navigation" className="flex items-center gap-1">
              ...
              <button type="button" aria-expanded={isMenuOpen} aria-controls="mobile-navigation" aria-label={isMenuOpen ? 'Close menu' : 'Open menu'} onClick={() => setIsMenuOpen((open) => !open)} className="flex h-11 w-11 items-center justify-center rounded-full border border-slate-200 bg-white/60 transition-colors hover:bg-white sm:hidden">
        ...
        >
          <div className="min-h-0">
            <div className="mx-auto flex max-w-7xl gap-2 px-5 py-3">
              <a onClick={() => setIsMenuOpen(false)} href="#catalog" className="flex min-h-11 flex-1 items-center justify-center rounded-full bg-slate-100 text-sm font-medium text-slate-800">Catalog</a>
              <a onClick={() => setIsMenuOpen(false)} href="#standards" className="flex min-h-11 flex-1 items-center justify-center rounded-full bg-slate-100 text-sm font-medium text-slate-800">Standards</a>
            </div>
          </div>
        </nav>
      </header>
      ...
      <main id="top" className="mx-auto max-w-7xl px-5 pb-24 sm:px-8">
        <section className="relative overflow-hidden py-16 sm:py-24">
          ...
          <div className="relative max-w-3xl">
            <div className="reveal-load mb-7 flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.22em] text-indigo-700" style={{ animationDelay: '40ms' }}>
              <Sparkles className="h-3.5 w-3.5" /> Curated research supply
            </div>
            ...
            <p className="reveal-load mt-8 max-w-xl text-base leading-7 text-slate-600 sm:text-lg" style={{ animationDelay: '260ms' }}>
              A considered collection of research compounds, presented with clarity and handled with care.
            </p>
            <a
              href="#catalog"
              className="reveal-load mt-9 inline-flex min-h-11 items-center gap-2 rounded-full bg-slate-950 px-5 text-sm font-medium text-white transition-transform hover:translate-y-[-2px]"
              style={{ animationDelay: '380ms' }}
            >
              Explore catalog <ArrowDownRight className="h-4 w-4" />
            </a>
          </div>
        </section>
        ...
        <section id="catalog" className="scroll-mt-24">
          <Reveal>
            <div className="mb-7 flex items-end justify-between">
              <div>
                <p className="text-xs font-semibold uppercase tracking-[0.22em] text-indigo-700">The collection</p>
                <h2 className="mt-2 font-serif text-4xl tracking-[-0.04em] text-slate-950">Browse the catalog</h2>
              </div>
              <div className="hidden items-center gap-2 text-xs text-slate-500 sm:flex">
                <ShoppingBag className="h-4 w-4" /> {PRODUCTS.length} compounds
              </div>
            </div>
          </Reveal>
        ...
        <Reveal>
          <section id="standards" className="mt-16 scroll-mt-24 rounded-[1.5rem] bg-slate-950 p-7 text-white sm:p-10">
            <p className="text-xs uppercase tracking-[0.2em] text-indigo-200">A note on responsible research</p>
            <h2 className="mt-4 max-w-xl font-serif text-3xl leading-tight sm:text-4xl">Precision starts with knowing exactly what you are ordering.</h2>
            <p className="mt-4 max-w-2xl text-sm leading-6 text-slate-300">Products in this catalog are intended for research use only. Please review each detail sheet and all applicable handling and compliance requirements before ordering.</p>
          </section>
        </Reveal>
      </main>
