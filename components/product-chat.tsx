...
const suggestions = ['What is currently in stock?', 'How do I find product details?', 'What happens at checkout?']
...
export function ProductChat() {
  ...
  return <div className="fixed bottom-5 right-5 z-40 sm:bottom-7 sm:right-7">
    {open && <section aria-label="Product concierge" className="mb-3 flex h-[min(32rem,calc(100vh-7rem))] w-[min(22rem,calc(100vw-2.5rem))] flex-col overflow-hidden rounded-[1.5rem] border border-slate-200 bg-white/95 shadow-[0_24px_80px_-25px_rgba(15,23,42,.35)] backdrop-blur-xl">
      ...
      <div className="flex-1 space-y-3 overflow-y-auto p-4" aria-live="polite">{messages.map((message, index) => <div key={`${message.role}-${index}`} className={`max-w-[88%] rounded-2xl px-3 py-2 text-sm leading-6 ${message.role === 'user' ? 'ml-auto bg-slate-950 text-white' : 'bg-indigo-50 text-slate-700'}`}>{message.content}</div>)}{messages.length === 1 && <div className="flex flex-wrap gap-2 pt-1">{suggestions.map((suggestion) => <button key={suggestion} type="button" disabled={loading} onClick={() => sendQuestion(suggestion)} className="rounded-full border border-indigo-100 bg-white px-3 py-1.5 text-left text-xs font-medium text-indigo-700 hover:bg-indigo-50 disabled:opacity-50">{suggestion}</button>)}</div>}{loading && <div className="max-w-[88%] rounded-2xl bg-indigo-50 px-3 py-2 text-sm text-slate-500">Reviewing the catalog...</div>}</div>
